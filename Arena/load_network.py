    ...
    def __init__(self, args, h_size, num_frames, num_actions, rnn_cell, myScope):
        ...
        if not (args.a_t):
            ...
            self.rnn_outputs, self.rnn_state = tf.nn.dynamic_rnn( \
                inputs=self.convFlat, cell=rnn_cell, dtype=tf.float32, initial_state=self.state_in,
                scope=myScope + '_rnn')
...
class DQNAgent:
    ...
    def __init__(self, args, num_actions):
        ...
        self.q_network = Qnetwork(args=args, h_size=self.h_size, num_frames=self.num_frames,
                                  num_actions=self.num_actions, rnn_cell=cell, myScope="QNet")
        self.target_network = Qnetwork(args=args, h_size=self.h_size, num_frames=self.num_frames,
                                       num_actions=self.num_actions, rnn_cell=cellT, myScope="TargetNet")
        ...
        while idx_episode <= num_episodes:
            ...
            if np.random.random() < 1e-3:
                alpha_list = self.sess.run(self.q_network.alpha_list, \
                                           feed_dict={self.q_network.imageIn: action_state[None, :, :, :],
                                                      self.q_network.batch_size: 1})
...
# Pure Tensorflow implementation. Includes Basic Dueling Double DQN and Spatial Attention DQN.
# from src.attention_dqn.deeprl_prj.dqn_tf_spatialAt import DQNAgent
...
def main():
    ...
    parser.add_argument('--num_samples', default=100000000, type=int,
    ...
    parser.add_argument('--load_network_path', default='Arena/log/Seaquest-v0-run2-Recurrent_DQN', help='the path to the trained mode file')
    ...
    parser.add_argument('--num_episodes_at_test', default=20, type=int,
    ...
    parser.add_argument('--task_name', default='Recurrent_DQN', help='task name')
    parser.add_argument('--recurrent', default=True, dest='recurrent', action='store_true', help='enable recurrent DQN')
    parser.add_argument('--a_t', default=False, dest='a_t', action='store_true',
                        help='enable temporal/spatial attention')
    parser.add_argument('--global_a_t', default=False, dest='global_a_t', action='store_true',
                        help='enable global temporal attention')
    parser.add_argument('--selector', default=False, dest='selector', action='store_true',
                        help='enable selector for spatial attention')
    parser.add_argument('--bidir', default=False, dest='bidir', action='store_true',
                        help='enable two layer bidirectional lstm')
