...
class Qnetwork():
    def __init__(self, args, h_size, num_frames, num_actions, rnn_cell_1, myScope, rnn_cell_2=None):
        ...
        self.imageIn = tf.placeholder(shape=[None, SIZE_X, SIZE_Y, num_frames], dtype=tf.float32)
        self.image_permute = tf.transpose(self.imageIn, perm=[0, 3, 1, 2])
        self.image_reshape = tf.reshape(self.image_permute, [-1, SIZE_X, SIZE_Y, 1])
        self.image_reshape_recoverd = tf.squeeze(
            tf.gather(tf.reshape(self.image_reshape, [-1, num_frames, SIZE_X, SIZE_Y, 1]), [0]), [0])
...
class DQNAgent_temporalAttention:
    def __init__(self, path_model_to_load=None):
        ...
        self._action = None
        self.episode_number = 0
        self._decision_maker = decision_maker_DQN_temporalAttention(path_model_to_load)
        self.min_reward = -np.Inf
        self._type = AgentType.DQN_temporalAttention
