...
if __name__ == '__main__':
    ...
    for episode in tqdm(range(1, NUM_OF_EPISODES + 1), ascii=True, unit='episodes'):
        ...
        while (env.compute_terminal() is WinEnum.Done):
            env.reset_players_positions(episode)
        ...
        for steps_current_game in range(1, MAX_STEPS_PER_EPISODE + 1):
            ...
            whos_turn: Color = current_episode.whos_turn(steps_current_game)
            ...
            # Handle rewards
            reward = env.handle_reward(steps_current_game)
            reward_step_blue = 0
            reward_step_red = 0
            if whos_turn == Color.Blue:
                reward_step_blue = reward
                # update model
                blue_decision_maker.update_context(new_observation_for_blue,
                                                       reward_step_blue,
                                                       current_episode.is_terminal, EVALUATE)

            else: # Red players turn
                reward_step_red = reward
                # update model
                if RED_PLAYER_MOVES:
            ...
            if steps_current_game == MAX_STEPS_PER_EPISODE:
                # we reached MAX_STEPS_PER_EPISODE
                current_episode.is_terminal = True

            if current_episode.is_terminal:
                env.update_win_counters(steps_current_game, whos_turn)
                break
        ...
        if EVALUATE:
            print("\nEvaluation summury: num_episodes: ", episode, ", reward for blue: ",  current_episode.episode_reward_blue)
